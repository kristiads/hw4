...
//   component.
// - You will be receiving a warning message when the application loads, if the icon is
...
export default class App extends React.Component {
  ...
  async getWeather() {
    ...
    let responseCurrentIcon = response.weather.currently.icon;
    let FontAwesomeCurrentIcon = icon(responseCurrentIcon);

    // manipulate state
    this.setState({
      locationName: response.location,
      currentTemperature: Math.round(response.weather.currently.temperature) + String.fromCharCode(176) + "F",
  ...
  render() {
    ...
    if (this.state.forecast.length>0) {
      for (let i=1; i<6; i++) {
        ...
        let forecastUnixTime = this.state.forecast[i].time;
        let forecastDate = new Date(forecastUnixTime*1000);
        let forecastDay = weekday[forecastDate.getDay()];
